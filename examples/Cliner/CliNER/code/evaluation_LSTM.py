...
def assess_model(y_pred, y_true, labels, target_names, labels_with_o, target_names_with_o, evaluation_mode='bio', verbose=False):
    results = {}
...
#     (dataset.unique_label_indices_of_interest, dataset.unique_label_indices_of_interest).
    ...
    if evaluation_mode == 'bio':
        ...
        new_label_names.sort(key=lambda x: (hd.remove_bio_from_label_name(x), x))
