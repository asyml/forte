...
box_score_words: Dict[str, Any] = {
    "PLAYER-{}".format(name): value for name, value in box_score_words.items()
}
...
def get_ents(dat):
    ...
    for entset in [players, teams, cities]:
        for k in list(entset):
            ...
            if len(pieces) > 1:
                for piece in pieces:
                    if len(piece) > 1 and piece not in [
                        "II",
                        "III",
                        "Jr.",
                        "Jr",
...
def deterministic_resolve(
    pron, players, teams, cities, curr_ents, prev_ents, max_back=1
):
    ...
    while i < len(sent):
        if sent[i] in prons:
            if resolve_prons:
                ...
                if referent is None:
                    sent_ents.append(
                        Ent(i, i + 1, sent[i], True)
                    )  # is a pronoun
                else:
                    # pretend it's not a pron and put in matching string
                    sent_ents.append(Ent(i, i + 1, referent[2], False))
            else:
...
def get_rels(
    entry, tokens, ents, nums, players, teams, cities, filter_none=False
):
    """
    ...
    for i, ent in enumerate(ents):
        ...
        if (
            ...
            is_home = None
            if (
                entpieces[0] in entry["home_city"]
                or entpieces[-1] in entry["home_name"]
            ):
                linescore = entry["home_line"]
                is_home = True
            elif (
                entpieces[0] in entry["vis_city"]
                or entpieces[-1] in entry["vis_name"]
            ):
    ...
    filt = (
        (lambda cond, rels: filter(cond, rels))
        if filter_none
        else (
            lambda cond, rels: map(
                lambda rel: rel
                if cond(rel)
                else Rel(rel.ent, rel.num, "NONE", None),
                rels,
            )
        )
    )
...
def do_connect_multiwords(tokes, rels, ents, nums):
    ...
    for rel in rels:
        ...
        assert (
            new_tokens[ent.start] == ent.s
        ), "new_tokens = {}, ent = {}".format(new_tokens, ent)
        num = rel.num
        num = Num(start=new_loc[num.start], end=new_loc[num.end], s=num.s)
...
def get_multilabeled_data(tup, vocab, labeldict, max_len):
    ...
    for rel, label_list in unique_rels.items():
        ...
        yield sent, sentlen, ent_dists, num_dists, [
            labeldict[label] for label in label_list
        ]
...
def candidate_rels_extractor(
    all_ents, players, teams, cities, prons, connect_multiwords=False
):
    def extract(entry, summ, filter_none=False):
        return get_candidate_rels(
            entry,
            summ,
            all_ents,
            prons,
...
def get_datasets(path, connect_multiwords=False, filter_none=False):
    ...
    extract = candidate_rels_extractor(
        all_ents,
        players,
        teams,
        cities,
    ...
    for stage, dataset in datasets.items():
        ...
        nugz = list(
            filter(lambda data: len(data[0]) <= 50 and len(data[1]) <= 50, nugz)
        )
...
def save_full_sent_data(
    ...
    datasets = get_datasets(
        path, connect_multiwords=not backup, filter_none=False
    )[0]
    ...
    if nonedenom > 0:
        ...
        none_idxs = [
            i
            for i, labellist in enumerate(trlabels)
            if labellist[0] == labeldict["NONE"]
        ]
        ...
        stuffs["train"] = [
            thing
            for i, thing in enumerate(stuffs["train"])
...
def extract_sentence_data(
    outfile_prefix, path, connect_multiwords=True, filter_max_len=50
):
    ...
    for stage, dataset in datasets.items():
        ...
        corpus = filter(
            lambda pair: 0 < len(pair[0]) <= filter_max_len
            and len(pair[1]) <= filter_max_len,
            corpus,
        )
        ...
        src_lines = map(
            lambda triples: " ".join(
                "|".join((t[2], t[0], t[1])) for t in triples
            ),
            src_lines,
        )
...
def prep_generated_data(
    genfile, dict_pfx, outfile, trdata, val_file, rec_outfile=None, backup=False
):
    ...
    if not backup:
        all_ents, players, teams, cities = (
            set(x.replace(" ", "_") for x in ents)
            for ents in (all_ents, players, teams, cities)
        )
        extract = candidate_rels_extractor(
            all_ents, players, teams, cities, prons
        )
    ...
    with open(val_file, "r") as f:
        valdata = (
            json.load(f)
            if backup
            else [[x.split("|") for x in line.split()] for line in f]
        )

    assert len(valdata) == len(
        gens
    ), "len(valdata) = {}, len(gens) = {}".format(len(valdata), len(gens))
    ...
    if not backup:
        ...
        if rec_outfile is not None:
            line_format = "\t".join
            print(
                line_format(("gold_n", "gold_match_n", "pred_nonnone_n")),
                file=rec_outfile,
            )
        ...
        ret = divide_or_const(sums[1], sums[0]), divide_or_const(
            sums[1], sums[2]
        )
    ...
    h5fi["boxrestartidxs"] = np.array(
        np.array(rel_reset_indices), dtype=int
    )  # 1-indexed
    h5fi.close()
...
def make_pointerfi(outfi, trdata, resolve_prons=False):
    ...
    for i, entry in enumerate(trdata):
        ...
        inv_vis_players = {
            pkey: (jj + NUM_PLAYERS) for jj, pkey in enumerate(vis_players)
        }
                    ...
                    if isinstance(idthing, bool):  # city or team
                        ...
                        for k, word in enumerate(tokes[ent_start:ent_end]):
                            ...
                            if src_idx is not None:
                                ...
                                if (
                                    targ_idx >= len(entry["summary"])
                                    or entry["summary"][targ_idx] != word
                                ):
                                ...
                                if targ_idx is None:
                                    ...
                                    assert (
                                        rulsrcs[i][src_idx] == word
                                        and entry["summary"][targ_idx] == word
                                    )
                        ...
                        if player_row is not None:
                            ...
                            for k, word in enumerate(tokes[ent_start:ent_end]):
                                ...
                                if (
                                    word
                                    == entry["box_score"]["FIRST_NAME"][idthing]
                                ):
                                    # second to last thing
                                    src_idx = (player_row + 1) * (
                                        len(bs_keys) - 1
                                    ) - 2
                                elif (
                                    word
                                    == entry["box_score"]["SECOND_NAME"][
                                        idthing
                                    ]
                                ):
                                    src_idx = (player_row + 1) * (
    ...
    with open(outfi, "w+") as f:
        for links_dict in trlink_dicts:
            ...
            fmtd = [
                ",".join(
                    [str(targ_idx)]
                    + [str(thing) for thing in set(links_dict[targ_idx])]
                )
...
def save_coref_task_data(outfile, in_file="full_newnba_prepdata2.json"):
        ...
        for i, entry in enumerate(dataset):
            ...
            for j in range(1, len(ents)):
                ...
                if curr_pron:  # prons
                    ...
                    label = (
                        2 if prev_str in curr_str or curr_str in prev_str else 1
                    )
    ...
    trwindows = [
        ...
        + [-1] * (max_trlen - len(window))
        for (window, label) in datasets[0]
    ]
    trlabels = [label for (window, label) in datasets[0]]
    valwindows = [
        [vocab[wrd] if wrd in vocab else vocab["UNK"] for wrd in window]
        + [-1] * (max_vallen - len(window))
        for (window, label) in datasets[1]
    ]
    ...
    h5fi["trlens"] = np.array(
        [len(window) for (window, label) in datasets[0]], dtype=int
    )
    h5fi["trlabels"] = np.array(trlabels, dtype=int)
    h5fi["valwindows"] = np.array(valwindows, dtype=int)
    h5fi["vallens"] = np.array(
        [len(window) for (window, label) in datasets[1]], dtype=int
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_path", type=str, default="rotowire", help="path to input"
    )
    parser.add_argument(
        "--output", type=str, default="", help="desired path to output file"
    )
    parser.add_argument(
        "--gen",
        type=str,
    ...
    if args.mode == "ptrs":
        ...
        save_full_sent_data(
            args.output, path=args.input_path, multilabel_train=True
        )
