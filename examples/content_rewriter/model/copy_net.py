...
def update_coverity(coverity, probs, h, cell):
    ...
    h = tf.broadcast_to(
        tf.expand_dims(h, 1), tf.concat([shape, [h.shape[-1]]], -1)
    )
...
class CopyNetWrapperState(
    ...
    def clone(self, **kwargs):
        ...
        return nest.map_structure(
            with_same_shape, self, super()._replace(**kwargs)
        )
...
class CopyNetWrapper(tf.nn.rnn_cell.RNNCell):
    def __init__(
        ...
        coverity_dim=None,
        coverity_rnn_cell_hparams=None,
        ...
        with tf.variable_scope("CopyNetWrapper", reuse=reuse):
            ...
            self._memory_ids_states_lengths = [
                ...
                for memory_ids, memory_states, memory_lengths in memory_ids_states_lengths
            ]
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        if self._coverage:
            coverities = [
                tf.reshape(
                    coverity, [tf.shape(coverity)[0], -1, self._coverity_dim]
                )
                for coverity in state.coverities
            ]

        def _get_selective_read(
            memory_ids, memory_states, memory_lengths, prob
        ):
            int_mask = tf.cast(
                tf.equal(tf.expand_dims(last_ids, 1), memory_ids), tf.int32
            )
        ...
        inputs = [inputs]
        for (memory_ids, memory_states, memory_lengths), copy_prob in zip(
            self._memory_ids_states_lengths, state.copy_probs
        ):
            inputs.append(
                _get_selective_read(
        ...
        copy_scores = self._get_copy_scores(
            outputs, coverities=coverities if self._coverage else None
        )
        exp_copy_scores = [
            tx.utils.mask_sequences(
                tf.exp(tf.cast(copy_score, tf.float64)), memory_lengths
            )
            for (_, _, memory_lengths), copy_score in zip(
                self._memory_ids_states_lengths, copy_scores
            )
        ]
        ...
        disabled_probs = self._eps * tf.ones(
            tf.concat(
                [tf.shape(probs_generate)[:-1], [self._disabled_vocab_size]], -1
            ),
            dtype=tf.float64,
            name="disabled_probs",
        )
            ...
            indices = tf.stack(
                [
                    tf.tile(
                        tf.expand_dims(
                            tf.range(
                                tf.cast(batch_size, tf.int64), dtype=tf.int64
                            ),
                            axis=-1,
                        ),
        ...
        if self._coverage:
            new_coverities = []
        for i, ((memory_ids, _, _), exp_copy_score) in enumerate(
            zip(self._memory_ids_states_lengths, exp_copy_scores)
        ):
            ...
            if self._coverage:
                new_coverity = update_coverity(
                    coverities[i],
                    tf.cast(copy_prob, tf.float32),
                    outputs,
                    self._coverity_rnn_cells[i],
                )
                new_coverity = tf.reshape(
                    new_coverity, [tf.shape(new_coverity)[0], -1]
                )
