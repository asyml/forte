...
class Rewriter():
    def __init__(self):
        ...
        self.train_ops, self.bs_outputs, _, _, _, _ \
            = build_model(self.data_batch, self.datasets['train'], self.global_step)
    ...
    def eval_epoch(self, mode):
        ...
        fetches = [
            ...
            [self.data_batch['x_value_text'], self.data_batch['x_ref_value_text']],
        ...
        while True:
            try:
                ...
                output_texts = tx.utils.map_ids_to_strs(
                    ids=output_ids.tolist(), vocab=self.datasets[mode].vocab('y_aux'),
                    join=False)
