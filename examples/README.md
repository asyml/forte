...
`forte.pipeline` is a class functions as its name indicates. User need to add
...
```
* processors/models can be from many well-developed external open-source pacakages such as Huggingface, Spacy and nltk.
...
* We iterate data by `pipeline.process_dataset(input_html)`.
