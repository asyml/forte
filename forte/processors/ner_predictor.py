...
class BioBERTNERPredictor(FixedSizeBatchProcessor):
    ...
    def predict(self, data_batch: Dict[str, Dict[str, List[str]]]) \
        ...
        labels_idx = score.argmax(axis=-1)[:, 1:-1] # Remove first and last placeholder
    ...
    def _fill_sub_entities(self, entity_groups_disagg, data_pack, tids): 
        ...
        while 0 < first_idx and data_pack.get_entry(first_tid).is_subword:
            first_idx -= 1
            first_tid -= 1
    ...
    def _group_entities(self, entities, data_pack, tids): 
        """
        Find and group together adjacent tokens considered to have the same entity
        Logic: An entity starts with a "B" and extends to the end of the word that contains last "I"    
        """
        entity_groups = []
