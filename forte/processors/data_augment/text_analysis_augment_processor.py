...
__all__ = [
    "TextAnalysisDataAugmentProcessor"
]

random.seed(0)

class TextAnalysisDataAugmentProcessor(ReplacementDataAugmentProcessor):
    r"""
    The data augmentation processor for text analysis data.
    """
    ...
    def _process(self, multipack: MultiPack):
        r"""
        This function assumes a single input data pack from the input multipack,
        then generate one or more augmented data pack,
        then add the augmented data pack(s) to the multipack.
        """
        ...
        # Create aug_num pairs of augmented datapacks
        for i in range(self.configs.aug_num):
            aug_pack: DataPack = self._process_pack(multipack.get_pack(self.configs.input_pack_name))
            multipack.update_pack({
                "{}_{}".format(self.configs.aug_pack_name, str(i)): aug_pack,
            })
