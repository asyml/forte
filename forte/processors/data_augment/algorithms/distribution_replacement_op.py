...
from forte.processors.data_augment.algorithms.sampler import UniformSampler, UnigramSampler
import requests
...
class DistributionReplacementOp(TextReplacementOp, Configurable):
    ...
    Args:
        ...
        configs: The config should contain the following:
            - `prob`
            ...
            - `distribution_path`
                ...
                The data must be stored in json file as list for uniform sampler and dictionary for unigram sampler
            - `distribution_data`
                If the data is to be passed directly, it is passed (either as a list or dict depending on the type of sampler)
    """
    ...
    def cofigure_sampler(self) -> bool:
        r"""
        This function sets the sampler (Unigram or Uniform) that will be used distribution replacement op.
        ...
        if sampler_type not in {"uniform", "unigram"}:
            raise ValueError("The value of 'sampler_type' has to be one of ['uniform', 'unigram'].")
        ...
        if distribution_path:
            ...
            data = self.configs['unigram_sampler_data'].__dict__["_hparams"] if sampler_type == "unigram" else self.configs['uniform_sampler_data']
        ...
        if sampler_type == "uniform":
            ...
            self.sampler = UniformSampler(
                configs={
                    "uniform_sampler_word_list": data
                }
            )
            return True
        if sampler_type == "unigram":
            if not isinstance(data, dict):
                raise TypeError("The input for unigram sampler must be a dictionary")
            self.sampler = UnigramSampler(
                configs={
                    "unigram_dict": {"type": "", "kwargs": data}
                }
            )
            return True
        return False
