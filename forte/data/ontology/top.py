...
class Annotation(Entry):
    ...
    def get(
        ...
        Args:
            ...
            include_sub_type: whether to consider the subtypes of
...
class Group(BaseGroup[Entry]):
    ...
    def get_members(self) -> List[Entry]:
        ...
        a list of member entries' ``tid``s from ``DataStore`` and convert them
...
class Box(Region):
    ...
    def compute_iou(self, other) -> float:
        ...
        (unit: pixel).
        It overwrites the ``compute_iou`` function in its parent class
        ``Region``.
