...
class DataPack(BasePack[Entry, Link, Group]):
    ...
    def get_entries(self,
        ...
        def _merge_sets(a: Optional[Set[int]], b: Iterable[int]) -> Set[int]:
            if a is not None:
                a &= b
            else:
                a = b
        ...
        if components is not None:
            ...
            for component in components:
                valid_component_id |= self.get_ids_by_component(component)
            valid_id = _merge_sets(valid_id, valid_component_id)

        # valid span
        if (range_annotation is not None and
                (self.index.coverage_index_is_valid or
                 issubclass(entry_type, Annotation))):
            # TODO: Also build indexes for Link and Group, which can be handled
            #   in the same way as Annotations.
            # Faster code path.

            if self.index.coverage_index_is_valid:
                coverage_index = self.index.coverage_index(
                    type(range_annotation), entry_type)
                assert coverage_index is not None
                valid_span_id = coverage_index[range_annotation.tid]
            else:
                ...
                valid_span_id = (
                    ...
                    if self.index.in_span(annotation, range_annotation.span))

            valid_id = _merge_sets(valid_id, valid_span_id)
            for entry_id in valid_id:
                entry = self.get_entry(entry_id)
                if isinstance(entry, entry_type):
                    yield entry
        else:
            valid_type_id = self.get_ids_by_type(entry_type)
            valid_id = _merge_sets(valid_id, valid_type_id)

            if range_annotation is not None:
                for entry_id in valid_id:
                    entry = self.get_entry(entry_id)
                    if self.index.in_span(entry, range_annotation.span):
                        yield entry
            else:
                yield from (self.get_entry(entry_id) for entry_id in valid_id)
