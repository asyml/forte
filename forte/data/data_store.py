...
class DataStore(BaseStore):
    ...
    def __init__(self, onto_file_path: Optional[str] = None):
        r"""An implementation of the data store object that mainly uses primitive types. This
        class will be used as the internal data representation behind data pack. The usage of
        ...
        particular type, such as entries for "ft.onto.base_ontology.Sentence". Different 
        types are sotred in different lists: [ <Document List>, <Sentence List>, ...]. We
        will discuss the sorting order later.
        
        The outer list, stores a list of `entry lists`, and each `entry_list` is indexed by 
        the type of its element. Specifically, each type is associated with a unique 
        `type_id`, which is generated by the system.
        
        Entry information is stored as `entry data` in each `entry list`. Each element in
        the `entry list` (an entry data) corresponds to one entry instance. 
        ...
        Here, `entry_type` is the fully qualified name of every entry. `tid` is an unique
        id of every entry, which is interally generated by uuid.uuid4().
        Each `entry_type` corresponds to a pre-defined ordered list of attributes, the exact
        order is determined by the system through the ontology specifications.
        ...
        `onto_file_path` is an optional argument, which allows one to pass in an user
        defined ontology file. This will enable the DataStore to understand and store 
        `entry_type` defined in the provided file.
    ...
    def delete_entry(self, tid: int):
        ...
        # entry from `__entry_dict` when its information is retrieved. We find the
