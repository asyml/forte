...
class DataStore(BaseStore):
    ...
    def __init__(self, onto_file_path: Optional[str] = None):
        ...
        We store every entry in a data structure `__elements`, which is a list of `entry lists`.
        ...
        `entry_type` is the fully qualified name of every entry. `tid` is an unique
        id of every entry, usually generated by uuid.uuid4().
        Each `entry_type` has a pre-defined ordered list of attributes, which is
        determined by the ontology specification.
        ...
        Annotation entries are sorted by the first two attributes (`begin`, `end`)
        of the entry.
