...
class DataStore(BaseStore):
    ...
    def __init__(self, onto_file_path: Optional[str] = None):
        ...
        `begin`, `end`, `tid` and `entry_type`. `begin` and `end`, which are
        ...
        `entry_type` and the order of corresponding attributes except `index_id`.
        ...
        is the index of each list in `self.__elements`.
        ...
        4345314235: [parent_tid, child_tid, tid, entry_type, attr_1, ...,
                    attr_n, index_id]}
        """
    ...
    def _new_link(self, entry_type: str, parent_tid: int, child_tid: int):
        r"""This function generates a new link with default fields.
        Called by add_link_raw() to create a new link with `entry_type`,
        ...
        tid: int = self._new_tid()
        entry = [parent_tid, child_tid, tid, entry_type]
        entry += len(self._type_attributes[entry_type]) * [None]
        return entry

    def _new_group(self, entry_type: str, member_type: str):
        r"""This function generates a new group with default fields.
        Called by add_group_raw() to create a new group with
        `entry_type` and `member_type`.
        ...
        tid: int = self._new_tid()
        entry = [member_type, [], tid, entry_type]
        entry += len(self._type_attributes[entry_type]) * [None]
        return entry
    ...
    def delete_entry(self, tid: int):
        ...
        # `entry_type`, `type_id`. If it is an Annotation-Like entry, we obtain
        # `begin` and `end` indices and use bisection to locate the `index_id`.
        # If it is not, we obtain the 'index_id' directly from `__entry_dict`.
        # We remove the entry from `__entry_dict` when its information is
        # retrieved. Then, the `type_id` and the `index_id` are passed to
