...
class DataStore(BaseStore):
    ...
    def __init__(self, onto_file_path: Optional[str] = None):
        ...
        """
        A dictionary that keeps record of indices of entries with their tid.
        Since entries are in different lists, they could have the same indices.
        It is a key-value map of {tid: index of the entry in the list}.

        e.g., {1423543453: 0, 5434514323: 1, 4323543451: 1}
        """
    ...
    def delete_entry(self, tid: int):
        ...
        # find the `entry_type` list using `type_id`. We use `__tid_idx` to
        # find the index of the entry. Then, the `type_id` and the index are
        # passed to `_delete_entry_by_loc`.
        raise NotImplementedError
    ...
    def get_entry(self, tid: int) -> Tuple[List, int, int]:
        ...
        # Otherwise, use `__tid_idx` to find the index of the entry.
        raise NotImplementedError
