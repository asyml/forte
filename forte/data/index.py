...
class BaseIndex(Generic[EntryType]):
    ...
    def update_anno_index(self, entries: List[Tuple], tids: List[int]):
        r"""Build or update the basic indexes, including
        ...
        for entry, tid in zip(entries, tids):
            self._entry_index[tid] = entry
            self._type_index[entry[0]].add(tid)
            # Disable sub type index since new items are added and this will
            #  be rebuilt in next query (`query_by_type_subtype`).
            self._subtype_index.pop(entry[0], None)
