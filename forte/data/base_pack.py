...
class BasePack(EntryContainer[EntryType, LinkType, GroupType]):
    ...
    def __init__(self, pack_name: Optional[str] = None):
        ...
        self.__meta: BaseMeta = self._init_meta(pack_name)
        self.__index: BaseIndex = BaseIndex()
    ...
    def set_meta(self, **kwargs):
        for k, v in kwargs.items():
            if not hasattr(self.__meta, k):
                raise AttributeError(f"Meta has no attribute named {k}")
            setattr(self.__meta, k, v)

    @property
    def pack_id(self):
        return self.__meta.pack_id
    ...
    def pack_name(self, pack_name: str):
        ...
        """
        self.__meta.pack_name = pack_name
    ...
    def get_entry(self, tid: int) -> EntryType:
        ...
        entry: EntryType = self.__index.get_entry(tid)
        if entry is None:
            raise KeyError(
    ...
    def get_ids_by_type(self, entry_type: Type[EntryType]) -> Set[int]:
        ...
        for index_key, index_val in self.__index.iter_type_index():
    ...
    def get_links_from_node(
        ...
        if not self.__index.link_index_on:
            self.__index.build_link_index(self._links)

        for tid in self.__index.link_index(tid, as_parent=as_parent):
            entry: EntryType = self.get_entry(tid)
    ...
    def get_groups_by_member(
        ...
        if not self.__index.group_index_on:
            self.__index.build_group_index(self._groups)

        for tid in self.__index.group_index(tid):
            entry: EntryType = self.get_entry(tid)
