...
class MonoFileReader(PackReader):
    ...
    def parse_pack(self, collection: Any) -> DataPack:
        ...
        Args:
            collection: The path to the original file to read.
        ...
        logger.info("reading from original file %s", collection)
        datapack = self.parse_pack(collection)
        self._record_fields(datapack)
        if not isinstance(datapack, DataPack):
            raise ValueError(
                f"No DataPack object read from the given "
                f"file path {collection}, returned {type(datapack)}."
            )
...
class MonoFileMultiPackReader(MultiPackReader):
    ...
    def iter(
            self,
            dir_path: str) -> Union[List[MultiPack], Iterator[MultiPack]]:
        """
