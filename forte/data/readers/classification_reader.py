...
# Q1: do we use the huggingface dataset class or we use the raw data (in json)
    # as the goal is the create some general classes for huggingface classification dataset, maybe it's better to use


def generate_text_n_anchor(subtext_names, **kwargs):
    """
    ...
    for i, sub_text in enumerate(kwargs):
        if not i:
            text += "\n"
        text += sub_text
        start = end + 1
...
class AmazonPolarityReader(PackReader):
    ...
    def _collect(self, csv_file: str) -> Iterator[Tuple[int, str]]:
        data = csv.reader(csv_file, delimiter=",", quoting=csv.QUOTE_ALL)
...
class Banking77(PackReader):
    ...
    """
    def _collect(self, csv_file: str) -> Iterator[Tuple[int, str]]:
        data = csv.reader(csv_file, delimiter=",", quoting=csv.QUOTE_ALL, skipinitialspace = True)
        next(data)
        for line_id, line in enumerate(data):
            yield line_id, line
    ...
    def _parse_pack(self, line_info: Tuple[int, str]) -> Iterator[DataPack]:
        ...
        pack.pack_name = line_id
        text, indices = generate_text_n_anchor(["sentence"], sentence)
        pack.set_text(text, replace_func=self.text_replace_operation)
        class_id = int(label)


        sent = Sentence(pack, 0, indices["sentence"][1])
    ...
    def default_configs(cls):
        ...
        config.update(
            {
                "class_names": dict(enumerate(class_list))
            }
        )
        return config
