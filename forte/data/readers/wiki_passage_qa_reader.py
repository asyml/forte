...
class WikiPassageQAReader(PackReader):
    ...
    DocInfoType = Tuple[bool, str, List[str], Optional[List]]
    ...
    def _collect(self, dir_path, split) -> Iterator[DocInfoType]:
        """
        ...
        if passage_mode:
            df["labels"] \
                = df.apply(lambda row_: _get_passage_ids(row_), axis=1)
        else:
