...
from forte.data.ontology.core import Entry, MpPointer
...
class MultiPack(BasePack[Entry, MultiPackLink, MultiPackGroup]):
    ...
    def get_entry(self, ptr: Union[BasePointer, int]) -> EntryType:
        r"""Look up the entry_index with key ``ptr``."""
        if isinstance(ptr, MpPointer):
            entry = self.packs[ptr.pack_index].get_entry(ptr.tid)
        else:
            entry = self.index.get_entry(ptr)

        if entry is None:
            raise KeyError(f"There is no entry with '{ptr}'' in this pack.")
        return entry
