...
class LabeledSpanGraphNetwork(tx.ModuleBase):
    ...
    def _filter_labels(
        ...
        srl_features: Optional[List[Feature]],
    ) -> torch.Tensor:
        ...
        if srl_features:
            for b_idx in range(batch_size):
                ...
                for e_idx, srl in enumerate(srl_data):
                    ...
                    assert (
                        srl_meta_data["parent_unit_span"][e_idx][1]
                        - srl_meta_data["parent_unit_span"][e_idx][0]
                        == 1
                    ), "predicate span larger than one"

                    srl_predicate = srl_meta_data["parent_unit_span"][e_idx][0]
                    span_idx = batch_spans[b_idx].get(
                        (srl_start, srl_end), None
                    )
    ...
    def forward(
        ...
        num_arguments = torch.ceil(
            self._hparams.argument_ratio * sent_lengths.float()
        ).long()
        ...
        num_predicates = torch.ceil(
            self._hparams.predicate_ratio * sent_lengths.float()
        ).long()
    ...
    def decode(
        ...
        result_dict = self.forward(
            text, char_batch, char_masks, text_batch, text_mask, None
        )
        sent_lengths = torch.sum(text_mask, dim=-1)
