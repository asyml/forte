...
def clean(args_):
    ...
    is_empty = generator.cleanup_generated_ontology(args_.dir)
    if not is_empty:
        log.info(f"Directory {args_.dir} not empty, cannot delete completely")


parser = argparse.ArgumentParser(description='Generate project ontology given '
                                             'a root JSON config.')
...
create_parser.add_argument('--dest_path',
                           ...
                           help='Destination directory provided by the user.' +
                                ' Only used when --no_dry_run is specified. ' +
