...
class DummyPackProcessor(PackProcessor):
    ...
    def _process(self, input_pack: DataPack):
        entries = list(input_pack.get_entries_by_type_subtype(NewType))
        if len(entries) == 0:
...
class DummmyFixedSizeBatchProcessor(FixedSizeBatchProcessor):
    ...
    def pack(self, data_pack: DataPack, output_dict: Optional[Dict] = None):
        entries = list(data_pack.get_entries_by_type_subtype(NewType))
        if len(entries) == 0:
...
class PipelineTest(unittest.TestCase):
    ...
    def test_pipeline1(self):
        ...
        num_packs = 0
        for pack in nlp.process_dataset(data_path):
            types = list(pack.get_entries_by_type_subtype(NewType))
            num_packs += 1
...
class MultiPackPipelineTest(unittest.TestCase):
    ...
    def test_pipeline1(self):
        ...
        num_packs = 0
        for pack in nlp.process_dataset(data_path):
            types = list(pack.get_pack("pack").get_entries_by_type_subtype(NewType))
            num_packs += 1
            self.assertEqual(len(types), 1)
            self.assertEqual(types[0].value, "[PACK]")
