...
class LargeMovieReaderTest(unittest.TestCase):
    def setUp(self):
        ...
        self.dataset_path: Dict[str, str] = {
            "pos": os.path.abspath(
                os.path.join(
                    file_dir_path, *([os.path.pardir] * 4), movie_dir, "pos"
                )
            ),
            "neg": os.path.abspath(
                os.path.join(
                    file_dir_path, *([os.path.pardir] * 4), movie_dir, "neg"
                )
            ),
        }
        ...
        self.doc_text: Dict[str, str] = {
            ...
            "neg": "this new imdb rule of requiring ten lines for every review "
        ...
        self.sent_text: Dict[str, str] = {
            ...
            "neg": "i was put through tears repulsion shock anger sympathy "
            "and misery when reading about the women of union street",
        }
        # pos0 doc's score, neg1 doc's score.
        self.score: Dict[str, float] = {"pos": 0.9, "neg": 0.1}
            ...
            for pack in data_packs:
                ...
                if pack.pack_name == docid1:
                    for doc in pack.get(Document):
                        ...
                        # test sentiments.
                        self.assertEqual(doc.sentiment[docid1], self.score[d])
                # Test sentences.
                elif pack.pack_name == docid0:
                    sents = pack.get(Sentence)
                    self.assertTrue(
                        self.sent_text[d] in [sent.text for sent in sents]
                    )
