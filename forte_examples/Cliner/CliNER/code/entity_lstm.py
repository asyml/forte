...
# import utils_tf
# import utils_nlp
from forte_examples.Cliner.CliNER.code import helper_dataset
from forte_examples.Cliner.CliNER.code import utils_tf


# TO DO: ADD CNN LAYER

def bidirectional_GRU(input, hidden_state_dimension, initializer,
        sequence_length=None, output_sequence=True):
    print("Biderectional GRU", initializer)
    with tf.variable_scope("biderectional_GRU"):
        ...
        for direction in ["forward", "backward"]:
            ...
            initial_state[direction] = gru_cell[direction].zero_state(
                batch_size, tf.float32)
        outputs, final_states = tf.nn.bidirectional_dynamic_rnn(
            gru_cell["forward"], gru_cell["backward"], input,
        ...
        if output_sequence:
            ...
            output = tf.concat([outputs_forward, outputs_backward], axis=2,
                name='output_sequence')

        else:
            final_states_forward, final_states_backward = final_states

            output = tf.concat([final_states_forward, final_states_backward],
                axis=1, name='output')  # 111
...
def bidirectional_LSTM(input, hidden_state_dimension, initializer,
    ...
    with tf.variable_scope("bidirectional_LSTM"):
            ...
            with tf.variable_scope(direction):
                ...
                lstm_cell[direction] = \
                    tf.contrib.rnn.CoupledInputForgetGateLSTMCell(
                        ...
                        state_is_tuple=True,
                        activation=tf.tanh)  # tf.tanh (default to RELU)
                ...
                initial_output_state = tf.get_variable("initial_output_state",
                    ...
                    initializer=initializer)
                c_states = \
                    tf.tile(initial_cell_state, tf.stack([batch_size, 1]))
                h_states = \
                    tf.tile(initial_output_state, tf.stack([batch_size, 1]))
                initial_state[direction] = \
                    tf.contrib.rnn.LSTMStateTuple(c_states, h_states)
        ...
        if output_sequence:
            ...
            final_states_forward, final_states_backward = final_states
            output = tf.concat([final_states_forward[1],
                                final_states_backward[1]], axis=1,
                name='output')
...
class EntityLSTM():
    ...
    def __init__(self, dataset, parameters):
        ...
        self.input_token_character_indices = \
            tf.placeholder(tf.int32, [None, None], name="input_token_indices")
        self.input_token_lengths = \
            tf.placeholder(tf.int32, [None], name="input_token_lengths")
        self.dropout_keep_prob = \
            tf.placeholder(tf.float32, name="dropout_keep_prob")

        self.input_features = \
            tf.placeholder(tf.float32, [None, self.feature_vector_length],
                name="features")
        ...
        if parameters['use_character_lstm']:
            with tf.variable_scope("character_embedding"):
                self.character_embedding_weights = tf.get_variable(
                    ...
                    initializer=initializer)
                embedded_characters = \
                    tf.nn.embedding_lookup(self.character_embedding_weights,
                        self.input_token_character_indices,
            ...
            with tf.variable_scope('character_lstm') as vs:
                if parameters['Use_LSTM']:
                    character_lstm_output = \
                        bidirectional_LSTM(embedded_characters,
                            parameters[
                                'character_lstm_hidden_state_dimension'],
                            initializer,
                            sequence_length=self.input_token_lengths,
                            output_sequence=False)
                    self.character_lstm_variables = \
                        tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,
                            scope=vs.name)
                else:
                    character_lstm_output = \
                        bidirectional_GRU(embedded_characters,
                            parameters[
                                'character_lstm_hidden_state_dimension'],
                            initializer,
                            sequence_length=self.input_token_lengths,
        ...
        if parameters['use_character_lstm']:
            with tf.variable_scope("concatenate_token_and_character_vectors"):
                token_lstm_input = \
                    tf.concat([character_lstm_output, embedded_tokens],
        ...
        if parameters['use_features_before_final_lstm']:
            with tf.variable_scope("features_argumentation_pre_LSTM"):
                token_lstm_input = \
                    tf.concat([token_lstm_input, self.input_features], 1)
                print(token_lstm_input)

        # Add dropout
        with tf.variable_scope("dropout"):
            token_lstm_input_drop = \
                tf.nn.dropout(token_lstm_input, self.dropout_keep_prob,
            ...
            token_lstm_input_drop_expanded = \
                tf.expand_dims(token_lstm_input_drop, axis=0,
        ...
        with tf.variable_scope('token_lstm') as vs:
            if parameters['Use_LSTM']:
                token_lstm_output = \
                    bidirectional_LSTM(token_lstm_input_drop_expanded,
                        parameters[
                ...
                token_lstm_output = \
                    bidirectional_GRU(token_lstm_input_drop_expanded,
                        parameters[
                            'token_lstm_hidden_state_dimension'],
                        initializer,
                        output_sequence=True)
            token_lstm_output_squeezed = tf.squeeze(token_lstm_output, axis=0)
            self.token_lstm_variables = \
                tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,
                    scope=vs.name)
        ...
        with tf.variable_scope("feedforward_after_lstm") as vs:
            ...
            b = tf.Variable(tf.constant(0.0,
                shape=[parameters[
                           'token_lstm_hidden_state_dimension']]),
                name="bias")
            outputs = tf.nn.xw_plus_b(token_lstm_output_squeezed, W, b,
        ...
        with tf.variable_scope("feedforward_before_crf") as vs:
            ...
            self.predictions = tf.argmax(self.unary_scores, 1,
                name="predictions")
            # utils_tf.variable_summaries(W)
            # utils_tf.variable_summaries(b)
            self.feedforward_variables = \
                tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,
                    scope=vs.name)
        ...
        if parameters['use_crf']:
            ...
            with tf.variable_scope("crf") as vs:
                ...
                unary_scores_expanded = tf.expand_dims(self.unary_scores,
                    axis=0,
                ...
                self.crf_variables = tf.get_collection(
                    ...
                    scope=vs.name)  # LATER FOR RESTORE
        ...
        # Do not use CRF layer
            ...
            with tf.variable_scope("accuracy"):
                correct_predictions = tf.equal(self.predictions,
                    tf.argmax(
                        self.input_label_indices_vector,
                        1))
    ...
    def define_training_procedure(self, parameters):
        ...
        if parameters['optimizer'] == 'adam':
            ...
            raise ValueError('The lr_method parameter must be \
            either adadelta, adam or sgd.')
        ...
        if parameters['gradient_clipping_value']:
            ...
            grads_and_vars = [(ClipIfNotNone(grad), var)
                              for grad, var in grads_and_vars]

        self.train_op = self.optimizer.apply_gradients(
            grads_and_vars, global_step=self.global_step)

    def load_pretrained_token_embeddings(self, sess, dataset,
        ...
        for token in dataset.token_to_index.keys():
            if token in token_to_vector.keys():
                initial_weights[dataset.token_to_index[token]] = \
                    token_to_vector[token]
                number_of_token_original_case_found += 1
            elif parameters['check_for_lowercase'] and token.lower() in \
                    token_to_vector.keys():
                initial_weights[dataset.token_to_index[token]] = \
                    token_to_vector[token.lower()]
                number_of_token_lowercase_found += 1
            elif parameters['check_for_digits_replaced_with_zeros'] \
                    and re.sub('\\d', '0',
                token) in token_to_vector.keys():
                initial_weights[dataset.token_to_index[token]] = \
                    token_to_vector[re.sub('\\d', '0', token)]
                number_of_token_digits_replaced_with_zeros_found += 1
            elif parameters['check_for_lowercase'] and \
                    parameters['check_for_digits_replaced_with_zeros'] and \
                    re.sub(
    ...
    def load_embeddings_from_pretrained_model(self, sess, dataset,
            pretraining_dataset,
        ...
        print('Load {0} embeddings from pretrained model... '.format(
            embedding_type), end='', flush=True)
        initial_weights = sess.run(embedding_weights.read_value())

        if embedding_type == 'token':
            initial_weights[dataset.UNK_TOKEN_INDEX] = \
                pretrained_embedding_weights[
                    pretraining_dataset.UNK_TOKEN_INDEX]
        elif embedding_type == 'character':
            initial_weights[dataset.PADDING_CHARACTER_INDEX] = \
                pretrained_embedding_weights[
        ...
        for index, string in index_to_string.items():
            ...
            if string in pretraining_string_to_index.keys():
                initial_weights[index] = \
                    pretrained_embedding_weights[
                        pretraining_string_to_index[string]]
                number_of_loaded_vectors += 1
        elapsed_time = time.time() - start_time
        print('done ({0:.2f} seconds)'.format(elapsed_time))
        print("number_of_loaded_vectors: {0}".format(number_of_loaded_vectors))
        if embedding_type == 'token':
            print("dataset.vocabulary_size: {0}".format(
    ...
    def resize_without_redoing_model(self, parameters,
            new_dataset_vocab_size, sess):
        ""
        utils_tf.resize_tensor_variable(sess, self.token_embedding_weights,
            [new_dataset_vocab_size, parameters[
    ...
    def restore_from_pretrained_model(self, parameters, dataset, sess,
        ...
        temp_pretrained_dataset_adress = parameters['model_folder'] + \
                                         os.sep + "dataset.pickle"
        temp_pretrained_model_adress = parameters['model_folder'] + \
                                       os.sep + parameters['model_name']

        print(temp_pretrained_model_adress)

        if pretrained_dataset is None:
            pretraining_dataset = pickle.load(open(
                temp_pretrained_dataset_adress, 'rb'))
        else:
        ...
        if pretraining_dataset.index_to_token == dataset.index_to_token and \
                pretraining_dataset.index_to_character == \
                dataset.index_to_character:
            ...
            utils_tf.resize_tensor_variable(sess,
            ...
            utils_tf.resize_tensor_variable(sess,
                self.character_embedding_weights,
                [dataset.alphabet_size, parameters[
                    'character_embedding_dimension']])
            utils_tf.resize_tensor_variable(sess, self.token_embedding_weights,
                [dataset.vocabulary_size,
                 parameters[
            ...
            self.load_embeddings_from_pretrained_model(sess, dataset,
                pretraining_dataset,
                token_embedding_weights,
                embedding_type='token')
            self.load_embeddings_from_pretrained_model(sess, dataset,
        ...
        parameters = {'reload_character_embeddings': True,
                      'reload_character_lstm': True,
                      'reload_token_embeddings': True,
                      'reload_token_lstm': True, 'reload_feedforward': True,
                      'reload_crf': True}
